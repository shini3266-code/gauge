...
async function fetchSheetData(url) {
  ...
  if (!headerMap.ticker || !headerMap.price || !headerMap.bear || !headerMap.base || !headerMap.target) {
    throw new Error("시트 헤더를 인식하지 못했습니다. ticker/price/bear/base/target 컬럼을 확인하세요.");
  }
...
export default function Dashboard() {
  ...
  const [usingSample, setUsingSample] = useState(!SHEET_CSV_URL);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(e.message || "시트를 불러오지 못했습니다.");
      setUsingSample(true);
    } finally {
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
          ...
          <span style={{ fontSize: 11, color: "#6B7686", fontFamily: "'JetBrains Mono', monospace" }}>
            {SHEET_CSV_URL
              ? lastUpdated
                ...
                : "Syncing..."
              : "Sample data (no sheet connected)"}
          </span>
        </div>
        ...
        {error && (
          ...
          >
            ...
            <span>{error} (샘플 데이터를 표시 중입니다)</span>
          </div>
        )}
